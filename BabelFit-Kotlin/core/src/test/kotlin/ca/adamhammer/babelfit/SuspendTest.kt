...
class SuspendTest {
    ...
    fun `suspend function returns result`() = runBlocking {
        ...
        val api = BabelFitBuilder(SuspendTestAPI::class)
    ...
    @Test
    fun `suspend function works with babelFit DSL`() = runBlocking {
        val mock = MockAdapter.scripted(SimpleResult("dsl"))
        val instance = babelFit<SuspendTestAPI> {
            adapter(mock)
        }
    ...
    fun `suspend memorize stores result in memory`() = runBlocking {
        ...
        val instance = BabelFitBuilder(SuspendMemoryTestAPI::class)
            .setAdapterDirect(mock)
            .build()
    ...
    fun `suspend function propagates adapter exception`() {
        ...
        val ex = assertThrows(BabelFitException::class.java) {
            runBlocking { api.get() }
        }
