...
class DslAndFallbackTest {
    ...
    @Test
    fun `babelFit DSL with resilience and interceptor`() {
    ...
    @Test
    fun `babelFit DSL exposes memory through instance`() = runBlocking {
        val mock = MockAdapter.scripted("stored")
        val instance = babelFit<MemoryTestAPI> {
            adapter(mock)
        }
    ...
    fun `AiResponse fallback extracts type from Future generic`() {
        ...
        val api = BabelFitBuilder(FallbackResponseAPI::class)
    ...
    fun `suspend function triggers fallback adapter when primary fails`() = runBlocking {
        ...
        val api = babelFit<SuspendTestAPI> {
            adapter(failingMock)
            resilience {
                maxRetries = 0
    ...
    @Test
    fun `babelFit DSL without adapter throws BabelFitConfigurationException`() {
        assertThrows(BabelFitConfigurationException::class.java) {
            babelFit<SimpleTestAPI> { }
        }
    }
}
