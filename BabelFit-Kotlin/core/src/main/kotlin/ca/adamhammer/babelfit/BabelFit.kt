...
/**
 * The core dynamic proxy invocation handler for BabelFit interfaces.
 *
...
class BabelFit(
    ...
    override fun invoke(proxy: Any?, method: Method, args: Array<out Any>?): Any? {
        ...
        if (method.declaringClass == Object::class.java) {
            return when (method.name) {
                "toString" -> "BabelFitProxy[${klass.simpleName}]"
    ...
    private suspend fun buildContext(method: Method, args: List<Any>?, resultClass: KClass<*>): PromptContext {
        ...
        val request = BabelFitRequest(descriptor, memoryStore.getAll(), resultClass)
