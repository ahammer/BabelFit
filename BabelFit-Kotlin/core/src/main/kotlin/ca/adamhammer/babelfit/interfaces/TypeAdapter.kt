...
/**
 * Bridges an external POJO type [P] to a BabelFit-native mirror type [M].
 *
 * The mirror type must be a `@Serializable` data class annotated with `@AiSchema`
 * so that BabelFit's schema generation and serialization pipeline works unchanged.
 *
 * Register adapters via `BabelFitBuilder.typeAdapter(...)`. The BabelFit proxy will
