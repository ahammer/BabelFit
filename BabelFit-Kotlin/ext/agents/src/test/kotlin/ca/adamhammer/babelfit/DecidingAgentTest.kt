...
import ca.adamhammer.babelfit.test.assertMethodInvocationContains
...
class DecidingAgentTest {
    ...
    fun `decide returns AiDecision from scripted adapter`() {
        val expectedDecision = AiDecision(method = "analyze")
    ...
    @Test
    fun `decide extension function creates proper schema from BabelFitInstance`() {
        val decision = AiDecision(method = "get")
        val mock = MockAdapter.scripted(decision)
        ...
        // Create a BabelFitInstance wrapping SimpleTestAPI to test the decide() extension
        val apiMock = MockAdapter.scripted(SimpleResult())
        val targetInstance = BabelFitBuilder(ca.adamhammer.babelfit.test.SimpleTestAPI::class)
