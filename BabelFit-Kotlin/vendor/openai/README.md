# BabelFit OpenAI Adapter

The `babelfit-openai` module provides an `ApiAdapter` implementation for OpenAI's API. It translates BabelFit's `PromptContext` into OpenAI chat completion requests and handles native tool calling.

## Features

- **Native Tool Calling**: automatically converts BabelFit `ToolProvider` definitions into OpenAI function schemas and handles the multi-turn tool execution loop.
