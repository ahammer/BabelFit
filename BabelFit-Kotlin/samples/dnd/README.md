# BabelFit D&D Case Study

The `samples-dnd` module is a text-based Dungeons & Dragons adventure that demonstrates BabelFit's AI patterns. It uses the AI as both the Dungeon Master (DM) and the Player Characters (PCs) to build a multi-agent, stateful AI application.
...
The sample uses five key BabelFit features to create a cohesive game loop:

### 1. BabelFit Agents (`AutonomousAgent`)
